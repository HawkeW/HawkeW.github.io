...
> 本文主要学习和参考：[Cesium | 海量点的加载与性能优化](https://juejin.cn/post/6972420331982028837)，欢迎点击原文阅读。</br>
> 输出本篇文章主要是为记录测试和学习的过程，毕竟好久没有水一篇正经文章了。。。

## 测试说明

- 本次测试的主要数据来源： `pointList.json`文件中的10000条测试数据
- `drawMarkers`函数作为10000条数据的绘制入口，对每个点的坐标添加随机数处理， 避免点多次重合
...
- `drawMarkers`循环调用`entities.add`创建`billboard`
- 每次`drawMarkers`调用耗时在`530ms~650ms`之间

![performance-entity-all](/assets/img/11-cesium-ts-mass-marks/01-performance-entity-all.png)

### **渲染过程**

合计约5s

![performance-entity-render](/assets/img/11-cesium-ts-mass-marks/01-performance-entity-render.png)



## 优化：primitives添加方式

使用过高德地图API的应该知道，高德地图的常用标记点有`Marker`，以及海量点`MassMarks`。实际上，通过`Entities.add`添加的`billboard`， 与`Marker`类似，拥有更多的属性及方法可以被调用；那么`Cesium`是否可以创建一种压缩版的Marker，从而达到类似海量点的效果呢？答案是肯定的——通过`primitives.add`添加`BillboardCollection`，然后调用`BillboardCollection.add`创建`billboard`。
...
![performance-primitives-all](/assets/img/11-cesium-ts-mass-marks/02-performance-primitives-all.png)
...
![performance-primitives-create](/assets/img/11-cesium-ts-mass-marks/02-performance-primitives-create.png)
...
![performance-primitives-render](/assets/img/11-cesium-ts-mass-marks/02-performance-primitives-render.png)
...
![满载GPU](/assets/img/11-cesium-ts-mass-marks/03-full-gpu-usage.png)

- 100,000 挂在地图上の0
![100000 billboard图片](/assets/img/11-cesium-ts-mass-marks/04-billboards-100-000.png)